...
interface EditQRContentProps {
  ...
  onSave?: (updatedQR: any) => void
  onCancel?: () => void
}

export function EditQRContent({ qrCode, onSave, onCancel }: EditQRContentProps) {
  /* --- form state --- */
  const [qrName, setQrName] = useState(qrCode.name)
  const [qrType, setQrType] = useState<"static" | "dynamic">(qrCode.type)
  const [contentType, setContentType] = useState<ContentType>(qrCode.contentType)

  /* content states */
  const [url, setUrl] = useState(qrCode.url || "https://example.com")
  const [textContent, setTextContent] = useState(qrCode.textContent || "")
  const [menuContent, setMenuContent] = useState(qrCode.menuContent || "")
  const [formContent, setFormContent] = useState(qrCode.formContent || "")
  const [uploadedFile, setUploadedFile] = useState<MockFile | null>(qrCode.uploadedFile || null)
  const [isDragOver, setIsDragOver] = useState(false)
  const fileInputRef = useRef<HTMLInputElement>(null)

  /* customization */
  const [fgColor, setFgColor] = useState(qrCode.fgColor || "#4338ca")
  const [bgColor, setBgColor] = useState(qrCode.bgColor || "#ffffff")
  const [roundedCorners, setRoundedCorners] = useState(qrCode.roundedCorners || false)
  const [addLogo, setAddLogo] = useState(!!qrCode.logoFile)
  const [logoFile, setLogoFile] = useState<{ name: string; url: string } | null>(qrCode.logoFile || null)
  const logoInputRef = useRef<HTMLInputElement>(null)

  /* edit mode flag */
  const isStaticQR = qrType === "static"
  ...
  return (
    <div className="grid grid-cols-1 gap-6 xl:grid-cols-5">
      ...
      <div className="flex flex-col gap-6 xl:col-span-3">
        ...
        <div className={`rounded-xl border border-border bg-card shadow-sm ${isStaticQR ? "opacity-50" : ""}`}>
          <div className="border-b border-border px-6 py-4">
            ...
            <p className="text-sm text-muted-foreground">
              {isStaticQR
                ? "Content is locked for static QR codes"
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <input
                      ...
                      value={url}
                      onChange={(e) => !isStaticQR && setUrl(e.target.value)}
                      disabled={isStaticQR}
                      className={`w-full rounded-lg border border-input bg-background px-4 py-2.5 pr-10 text-sm text-foreground placeholder:text-muted-foreground/60 transition-colors focus:border-primary focus:outline-none focus:ring-2 focus:ring-ring/20 ${isStaticQR ? "cursor-not-allowed opacity-60 bg-muted" : ""}`}
                    />
                    {url.length > 0 && (
                ...
                <div className="space-y-3">
                  ...
                  {!uploadedFile ? (
                    <div
                      ...
                      onDrop={(e) => !isStaticQR && handleFileDrop(e)}
                      onClick={() => !isStaticQR && fileInputRef.current?.click()}
                      ...
                      } ${isStaticQR ? "cursor-not-allowed opacity-60 bg-muted" : ""}`}
                    >
              ...
              {contentType === "text" && (
                <div className="space-y-2">
                  ...
                  <textarea
                    ...
                    value={textContent}
                    onChange={(e) => !isStaticQR && setTextContent(e.target.value)}
                    disabled={isStaticQR}
                    className={`w-full resize-none rounded-lg border border-input bg-background px-4 py-2.5 text-sm text-foreground placeholder:text-muted-foreground/60 transition-colors focus:border-primary focus:outline-none focus:ring-2 focus:ring-ring/20 ${isStaticQR ? "cursor-not-allowed opacity-60 bg-muted" : ""}`}
                  />
                  <p className="text-xs text-muted-foreground">
              ...
              {contentType === "menu" && (
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    value={menuContent}
                    onChange={(e) => !isStaticQR && setMenuContent(e.target.value)}
                    disabled={isStaticQR}
                    className={`w-full rounded-lg border border-input bg-background px-4 py-2.5 text-sm text-foreground placeholder:text-muted-foreground/60 transition-colors focus:border-primary focus:outline-none focus:ring-2 focus:ring-ring/20 ${isStaticQR ? "cursor-not-allowed opacity-60 bg-muted" : ""}`}
                  />
                  <p className="text-xs text-muted-foreground">
              ...
              {contentType === "form" && (
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    value={formContent}
                    onChange={(e) => !isStaticQR && setFormContent(e.target.value)}
                    disabled={isStaticQR}
                    className={`w-full rounded-lg border border-input bg-background px-4 py-2.5 text-sm text-foreground placeholder:text-muted-foreground/60 transition-colors focus:border-primary focus:outline-none focus:ring-2 focus:ring-ring/20 ${isStaticQR ? "cursor-not-allowed opacity-60 bg-muted" : ""}`}
                  />
                  <p className="text-xs text-muted-foreground">
        ...
        {isStaticQR && (
          <div className="rounded-lg border border-amber-200 bg-amber-50 px-4 py-3">
            <p className="text-sm font-medium text-amber-900">
              ℹ️ Content is locked for static QR codes
            </p>
            <p className="mt-1 text-xs text-amber-800">
              Static QR codes have fixed content. You can only customize the appearance (colors, logo, corners).
            </p>
          </div>
        )}
          ...
          <div className="flex gap-3">
            {onCancel && (
              <button
                type="button"
                onClick={onCancel}
                className="flex items-center justify-center gap-2 rounded-lg border border-border bg-background px-6 py-2.5 text-sm font-medium text-foreground transition-colors hover:bg-accent hover:text-accent-foreground"
              >
                Cancel
              </button>
            )}
              ...
              onClick={() => {
                if (onSave) {
                  onSave({
                    ...
                    type: qrType,
                    contentType,
                    url: isStaticQR ? (qrCode.url || "") : url,
                    textContent: isStaticQR ? (qrCode.textContent || "") : textContent,
                    menuContent: isStaticQR ? (qrCode.menuContent || "") : menuContent,
                    formContent: isStaticQR ? (qrCode.formContent || "") : formContent,
                    uploadedFile: isStaticQR ? (qrCode.uploadedFile || null) : uploadedFile,
                    fgColor,
                    bgColor,
                    roundedCorners,
                    logoFile,
